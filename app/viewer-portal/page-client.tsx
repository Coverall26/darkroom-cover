...
export default function ViewerPortalPageClient() {
  ...
  // Check if admin came through visitor portal — they should stay on viewer-portal
  const loginPortal = (session?.user as any)?.loginPortal;
  const isAdminPortalUser = loginPortal === "ADMIN" || (!loginPortal && isAdmin);

  useEffect(() => {
    // Only redirect admin users who came through admin portal to /hub.
    // Admin users who came through visitor portal (loginPortal === "VISITOR")
    // should stay on viewer-portal and see their datarooms as a visitor.
    if (status === "authenticated" && isAdmin && isAdminPortalUser && !hasRedirected.current) {
      hasRedirected.current = true;
      router.replace("/hub");
    }
  }, [status, isAdmin, isAdminPortalUser, router]);

  // Admin users on visitor portal should see their datarooms, not be blocked
  const shouldFetchDatarooms = status === "authenticated" && (!isAdmin || !isAdminPortalUser);

  const { data, isLoading, error } = useSWR<ViewerDataResponse>(
    shouldFetchDatarooms ? "/api/viewer/my-datarooms" : null,
    fetcher
  );
  ...
  // Show loading if: session loading, data loading, admin being redirected to hub, or single-dataroom redirect
  const isAdminRedirecting = isAdmin && isAdminPortalUser;
  if (status === "loading" || isLoading || isAdminRedirecting || isRedirecting) {
    return (
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-3 sm:px-4 py-3 sm:py-4">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="text-sm sm:text-lg font-semibold truncate">FundRoom Investor Portal</h1>
