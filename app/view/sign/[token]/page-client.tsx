...
export default function SignDocument() {
  ...
  const { trackEngagement: trackEvent } = useTracking();
  ...
  const handleSubmit = async () => {
    ...
    if (!consentConfirmed) {
      toast.error("You must consent to electronic signatures before signing");
      return;
    }
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-gray-50">
        <div className="mb-8 text-center">
          <h2 className="text-xl font-bold text-gray-900">FundRoom Sign</h2>
          ...
          <div className="mx-auto flex max-w-5xl flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:py-4">
            <div className="flex items-center gap-2 sm:gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-gray-900 sm:text-base">FundRoom Sign</span>
              </div>
            ...
            <div className="flex items-center justify-between gap-2 sm:justify-end">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="sm" onClick={handleSubmit} disabled={isSubmitting || !consentConfirmed} className="px-2 sm:px-4">
                  {isSubmitting ? (
        ...
        <main className="mx-auto max-w-5xl px-2 py-4 sm:px-4 sm:py-8">
          <div className="flex flex-col-reverse gap-4 lg:grid lg:grid-cols-3 lg:gap-8">
                      ...
                      <div className="flex flex-col items-center p-4">
                        ...
                        >
                          <div className="relative inline-block shadow-lg">
                            ...
                            />
                            {/* Signature field overlays on current page */}
                            {fields
                              .filter((f) => f.pageNumber === currentPage)
                              .map((field) => {
                                const isSigned = field.type === "SIGNATURE" && signatureData && fieldValues[field.id];
                                return (
                                  <div
                                    key={field.id}
                                    style={{
                                    ...
                                    className={`${
                                      field.type === "SIGNATURE"
                                        ? isSigned
                                          ? "border border-green-300 bg-white/90 rounded"
                                          : "border-2 border-dashed border-blue-400 bg-blue-50/80 rounded cursor-pointer hover:bg-blue-100/90"
                                        : ""
                                    }`}
                                    onClick={() => {
                                      if (field.type === "SIGNATURE" && !isSigned) {
                                        // Scroll to signature section
                                        const sigSection = window.document.querySelector('[data-signature-section]');
                                        sigSection?.scrollIntoView({ behavior: 'smooth' });
                                      }
                                    }}
                                  >
                                    {field.type === "SIGNATURE" && isSigned && signatureData && (
                                      <div className="w-full h-full flex items-center justify-center p-0.5">
                                        <img src={signatureData} alt="Signature" className="max-w-full max-h-full object-contain" />
                                        <CheckCircle2Icon className="absolute top-0.5 right-0.5 h-3 w-3 text-green-600" />
                                      </div>
                                    )}
                                    {field.type === "SIGNATURE" && !isSigned && (
                                      <div className="w-full h-full flex items-center justify-center">
                                        <div className="flex flex-col items-center gap-0.5">
                                          <PenIcon className="h-3 w-3 text-blue-600" />
                                          <span className="text-[9px] font-medium text-blue-700">Sign here</span>
                                        </div>
                                      </div>
                                    )}
                                  </div>
            ...
            <div className="space-y-4 sm:space-y-6">
              <div className="rounded-lg bg-white p-4 sm:p-6 shadow" data-signature-section>
              ...
              <div className="rounded-lg bg-white p-4 shadow">
                <div className="flex items-start gap-3 min-h-[44px]">
                  ...
                  />
                  <Label
                    htmlFor="esign-consent-standalone"
        ...
        <footer className="border-t bg-white py-4">
          <div className="mx-auto max-w-5xl px-4 text-center">
            <p className="text-xs text-gray-400">
              Powered by FundRoom | Secure Document Signing
            </p>
          </div>
        </footer>
      </div>
