...
export async function GET(req: NextRequest) {
  ...
  const capitalCalls = periodTransactions.filter(
    (t) => t.type === "CAPITAL_CALL",
  );
  const distributions = periodTransactions.filter(
  ...
  const statement = {
    investor: {
      name: investor.entityName || auth.session.user.name || auth.email,
    ...
    capitalAccount: {
      ...
      ownershipPercentage: investor.fund
        ? Number(investor.fund.currentRaise) > 0
          ? (
              (totalFunded / Number(investor.fund.currentRaise)) *
              100
            ).toFixed(2)
          : "0.00"
        : "0.00",
...
interface StatementData {
  ...
  fund: {
    name: string;
    status: string | import("@prisma/client").$Enums.FundStatus;
    targetRaise: number;
    currentRaise: number;
  } | null;
  period: { type: string; year: number; startDate: string; endDate: string };
  capitalAccount: {
    totalCommitment: number;
    totalFunded: number;
    unfundedCommitment: number;
    netCapitalAccount: number;
  ...
  k1Status: { available: boolean; year: number; estimatedDate: string };
  generatedAt: string;
  statementId: string;
}

function generateStatementHTML(statement: StatementData): string {
            ...
            <table>
              ...
              <tbody>
                ${statement.transactions
                  ...
                  <tr>
                    <td>${formatDate(typeof t.date === "string" ? t.date : t.date.toISOString())}</td>
