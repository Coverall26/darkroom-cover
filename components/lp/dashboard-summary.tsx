...
export function DashboardSummary({
  ...
  const showDistributions = lpVisibility?.showDistributions !== false;
  const showCapitalCalls = lpVisibility?.showCapitalCalls !== false;
  ...
  return (
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <Card className="bg-gradient-to-br from-emerald-900/30 to-gray-800/50 border-emerald-700/30 card-hover-lift animate-fade-in-up stagger-1">
          <CardHeader className="pb-2 p-3 sm:p-4">
            <CardDescription className="text-emerald-300/70 flex items-center text-xs sm:text-sm">
              <Wallet className="h-4 w-4 mr-2 text-emerald-400" aria-hidden="true" />
          ...
          <CardContent className="p-3 pt-0 sm:p-4 sm:pt-0">
            ...
            <p className="text-emerald-400/60 text-xs mt-1">
              <span className="font-mono tabular-nums">{summary.activeFunds}</span> active fund{summary.activeFunds !== 1 ? "s" : ""}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-blue-900/30 to-gray-800/50 border-blue-700/30 card-hover-lift animate-fade-in-up stagger-2">
          <CardHeader className="pb-2 p-3 sm:p-4">
            <CardDescription className="text-blue-300/70 flex items-center text-xs sm:text-sm">
              <DollarSign className="h-4 w-4 mr-2 text-blue-400" aria-hidden="true" />
          ...
          <CardContent className="p-3 pt-0 sm:p-4 sm:pt-0">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-blue-400/60 text-xs font-mono tabular-nums">{fundedPercentage}%</span>
            </div>
          </CardContent>
        </Card>

        {/* Distributions card — removed from DOM when showDistributions is false */}
        {showDistributions && (
          <Card className="bg-gradient-to-br from-purple-900/30 to-gray-800/50 border-purple-700/30 card-hover-lift animate-fade-in-up stagger-3">
            <CardHeader className="pb-2 p-3 sm:p-4">
              <CardDescription className="text-purple-300/70 flex items-center text-xs sm:text-sm">
                <ArrowDownRight className="h-4 w-4 mr-2 text-purple-400" aria-hidden="true" />
        ...
        {/* Capital Calls card — removed from DOM when showCapitalCalls is false */}
        {showCapitalCalls && (
          <Card className={`bg-gradient-to-br card-hover-lift animate-fade-in-up stagger-4 ${
            ...
            <CardHeader className="pb-2 p-3 sm:p-4">
              <CardDescription className={`flex items-center text-xs sm:text-sm ${
                ...
                {summary.pendingCapitalCallsCount > 0 ? (
                  <AlertTriangle className="h-4 w-4 mr-2 text-amber-400" aria-hidden="true" />
                ) : (
                  <CheckCircle2 className="h-4 w-4 mr-2 text-gray-500" aria-hidden="true" />
                )}
              ...
              {summary.pendingCapitalCallsCount > 0 ? (
                <>
                  ...
                  <p className="text-amber-400/60 text-xs mt-1">
                    <span className="font-mono tabular-nums">{summary.pendingCapitalCallsCount}</span> pending
                  </p>
                </>
              ) : (
                <>
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        <Card className="bg-gray-800/40 border-gray-700/50">
          <CardContent className="p-3 sm:p-4 flex items-center gap-3">
            ...
            <div>
              <div className="text-lg sm:text-xl font-bold text-white font-mono tabular-nums">{documentsCount}</div>
