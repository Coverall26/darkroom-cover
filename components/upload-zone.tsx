...
import { encryptFileForUpload, isClientSideEncryptionSupported } from "@/lib/files/encrypt-file";
...
export default function UploadZone({
  ...
  const encryptionEnabled = teamSettings?.enableClientSideEncryption ?? false;
  const encryptionSupported = typeof window !== "undefined" && isClientSideEncryptionSupported();
      ...
      const uploadPromises = validatedFiles.valid.map(async (file, index) => {
        ...
        if (encryptionEnabled && encryptionSupported) {
          try {
            ...
            // Create a hash of the encryption key for verification (not the key itself)
            const keyBuffer = new TextEncoder().encode(encryptionResult.encryptionKey);
            ...
            fileToUpload = file;
            encryptionMetadata = { isClientEncrypted: false };
          }
        }
        ...
        if (!useTusUpload) {
          ...
          // Use TUS resumable upload (note: encryption not yet supported for TUS)
          const { complete } = await resumableUpload({
            file: encryptionMetadata.isClientEncrypted ? fileToUpload as File : file,
        ...
        const documentData: DocumentData = {
          ...
          ...(encryptionMetadata.isClientEncrypted && {
            isClientEncrypted: true,
            encryptionKeyHash: encryptionMetadata.encryptionKeyHash,
            encryptionIv: encryptionMetadata.encryptionIv,
            originalFileName: encryptionMetadata.originalFileName,
            originalFileSize: encryptionMetadata.originalFileSize,
          }),
        };
