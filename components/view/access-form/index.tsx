...
export type DEFAULT_ACCESS_FORM_TYPE = {
  ...
  _accreditationChecks?: Record<string, boolean>;
  name?: string | null;
...
export default function AccessForm({
  ...
  const isFormValid = () => {
    ...
    if (requireAccreditation && !data.hasConfirmedAccreditation) return false;
          ...
          <form className="space-y-4" onSubmit={onSubmitHandler} translate="no">
            ...
            {requireAccreditation ? (
              <AccreditationSection
                {...{ data, setData, brand }}
                accreditationType={accreditationType || "SELF_CERTIFICATION"}
