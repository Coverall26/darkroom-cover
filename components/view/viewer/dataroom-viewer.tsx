...
export default function DataroomViewer({
  ...
  const allDocumentsCanDownload = useMemo(() => {
    ...
    return documents.some((doc) => {
      if (!doc.versions?.[0] || doc.versions[0].type === "notion") return false;
  ...
  const folderEffectiveUpdatedAt = useMemo(() => {
    ...
    const calculateEffectiveUpdatedAt = (folderId: string): Date => {
      ...
      // Safe date creation - fallback to epoch if updatedAt is null/undefined
      let maxDate = folder.updatedAt ? new Date(folder.updatedAt) : new Date(0);

      // Check documents in this folder
      const docsInFolder = folderDocuments.get(folderId) || [];
      docsInFolder.forEach((doc) => {
        if (doc.versions && doc.versions.length > 0 && doc.versions[0].updatedAt) {
      ...
      ...(folders || [])
        ...
        .map((folder) => {
          ...
          const effectiveUpdatedAt =
            ...
            (folder.updatedAt ? new Date(folder.updatedAt) : new Date(0));
  ...
  const renderItem = (item: FolderOrDocument) => {
    if ("versions" in item) {
      const primaryVersion = item.versions?.[0];
      const isProcessing = primaryVersion
        ? ["docs", "slides", "pdf"].includes(primaryVersion.type) &&
