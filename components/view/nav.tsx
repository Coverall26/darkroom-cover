...
import FundRoomSparkle from "../shared/icons/fundroom-sparkle";
import {
  Breadcrumb,
  BreadcrumbItem,
  BreadcrumbLink,
...
export default function Nav({
  ...
  const previewToken = searchParams?.get("previewToken") ?? null;
          ...
          <div className="absolute inset-y-0 right-0 flex items-center space-x-2 pr-2 sm:static sm:inset-auto sm:ml-6 sm:space-x-4 sm:pr-0">
            ...
            {assistantEnabled ? (
              <Link href={`/view/${linkId}/chat`}>
                ...
                >
                  <FundRoomSparkle className="h-5 w-5" />
                </Button>
              </Link>
            ) : null}
            {allowDownload ? (
