...
export function useLastUsed() {
  ...
  useEffect(() => {
    const storedValue = localStorage.getItem("last_fundroom_login");
    if (storedValue) {
  ...
  useEffect(() => {
    if (lastUsed) {
      localStorage.setItem("last_fundroom_login", lastUsed);
    } else {
      localStorage.removeItem("last_fundroom_login");
    }
  }, [lastUsed]);
