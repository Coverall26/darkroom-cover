...
export default function LinkSuccessSheet({
  ...
  const marketingUrl = process.env.NEXT_PUBLIC_MARKETING_URL || (typeof window !== 'undefined' ? window.location.origin : 'https://fundroom.ai');
  const baseLinkUrl =
    link.domainId && link.slug
      ? `https://${link.domainSlug}/${link.slug}`
      : `${marketingUrl}/view/${link.id}`;
  const linkUrl = `${baseLinkUrl}${baseLinkUrl.includes("?") ? "&" : "?"}ref=direct`;
