...
export type DEFAULT_LINK_TYPE = {
  ...
  enableAccreditation: boolean; // accreditation gate
...
export default function LinkSheet({
  ...
  const applyPreset = (presetId: string) => {
    ...
    setData((prev) => {
      ...
      return {
        ...
        enableAccreditation: preset.enableAccreditation || prev.enableAccreditation,
        accreditationType: preset.accreditationType || prev.accreditationType,
        accreditationMessage: preset.accreditationMessage || prev.accreditationMessage,
  ...
  const handleSubmit = async (event: any, shouldPreview: boolean = false) => {
    ...
    if (currentLink) {
      ...
      trackFunnel({
        name: "funnel_first_link_shared",
        properties: { teamId: teamInfo?.currentTeam?.id ?? "", linkId: returnedLink.id },
      });
