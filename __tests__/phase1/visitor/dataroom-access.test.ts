...
// Covers: Public dataroom access, analytics, mobile, CTAs, permissions, folders
...
describe('Phase 1: Visitor/LP Side - Dataroom Access E2E', () => {
  ...
  const mockDocuments = [
    { id: 'doc-ppm', dataroomId: 'dataroom-bermuda', name: 'Private Placement Memorandum', type: 'application/pdf', orderIndex: 0, createdAt: new Date() },
    { id: 'doc-subscription', dataroomId: 'dataroom-bermuda', name: 'Subscription Agreement', type: 'application/pdf', orderIndex: 1, createdAt: new Date() },
    { id: 'doc-financials', dataroomId: 'dataroom-bermuda', name: 'Financial Projections', type: 'application/pdf', orderIndex: 2, createdAt: new Date() },
  ];
  ...
  describe('Dataroom Access - No Login Required', () => {
    it('should allow unauthenticated visitor to access public dataroom', async () => {
      (mockPrisma.link.findUnique as jest.Mock).mockResolvedValue({
        ...mockLink,
        dataroom: { ...mockDataroom, isPublic: true },
      });
  ...
  describe('Analytics Tracking - View Model', () => {
    it('should create view record with visitor audit data', async () => {
      ...
      const view = await mockPrisma.view.create({ data: viewAuditData });
    ...
    it('should track document page views for compliance', async () => {
      ...
      const view = await mockPrisma.view.findUnique({ where: { id: 'view-1' } });
  ...
  describe('Mobile Responsive - iPhone Simulation', () => {
    ...
    it('should create view record with mobile device info', async () => {
      ...
      const view = await mockPrisma.view.create({ data: mobileViewData });
    ...
    it('should track tablet device access', async () => {
      ...
      const view = await mockPrisma.view.create({ data: tabletViewData });
  ...
  describe('Responsive CTAs', () => {
    ...
    it('should hide CTA when fund is closed', async () => {
      ...
      const fund = await mockPrisma.fund.findUnique({ where: { id: 'fund-bermuda' } });
  ...
  describe('Permissions-Based Security', () => {
    it('should allow download when enabled on link', async () => {
      ...
      const link = await mockPrisma.link.findUnique({ where: { id: 'link-public-123' } });
    ...
    it('should respect NDA gate on private dataroom', async () => {
      ...
      const dataroom = await mockPrisma.dataroom.findUnique({ where: { id: 'dataroom-bermuda' } });
  ...
  describe('Test Example Data', () => {
    it('should display complete Test fund information', async () => {
      (mockPrisma.fund.findUnique as jest.Mock).mockResolvedValue({
        ...mockFund,
        name: 'Test Fund I',
        targetRaise: 5000000,
      ...
      expect(fund?.name).toBe('Test Fund I');
