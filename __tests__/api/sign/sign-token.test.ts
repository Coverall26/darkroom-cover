...
describe('/api/sign/[token]', () => {
  ...
  describe('GET - Fetch signing document', () => {
    it('returns 400 if token is missing', async () => {
      ...
      expect(JSON.parse(res._getData())).toEqual({ error: 'Token is required' });
    });
    ...
    it('returns 404 if token is invalid', async () => {
      ...
      expect(JSON.parse(res._getData())).toEqual({ error: 'Invalid or expired signing link' });
    });
    ...
    it('returns 410 if document is expired', async () => {
      ...
      expect(JSON.parse(res._getData())).toEqual({ error: 'This signing link has expired' });
    });
    ...
    it('returns 410 if document is voided', async () => {
      ...
      expect(res._getStatusCode()).toBe(410);
      expect(JSON.parse(res._getData())).toEqual({ error: 'This document has been voided' });
    });
    ...
    it('returns 400 if document is already completed', async () => {
      ...
      expect(res._getStatusCode()).toBe(400);
      expect(JSON.parse(res._getData())).toEqual({ error: 'This document has already been completed' });
    });
  ...
  describe('POST - Submit signature', () => {
    it('returns 404 if recipient not found for POST', async () => {
      ...
      expect(res._getStatusCode()).toBe(404);
      expect(JSON.parse(res._getData())).toEqual({ error: 'Invalid signing link' });
    });
    ...
    it('returns 400 if recipient already signed', async () => {
      ...
      expect(res._getStatusCode()).toBe(400);
      expect(JSON.parse(res._getData())).toEqual({ error: 'You have already signed this document' });
    });
  ...
  describe('Document Access', () => {
    ...
    it('includes team name in document response', async () => {
      ...
      expect(res._getStatusCode()).toBe(200);
      const data = JSON.parse(res._getData());
      expect(data.document.teamName).toBe('FundRoom GP');
    });
  });
