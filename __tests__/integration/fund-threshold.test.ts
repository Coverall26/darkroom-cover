...
describe("Fund Capital Call Threshold", () => {
  ...
  describe("Bulk Action Threshold Enforcement", () => {
    it("gates capital call when threshold not met", async () => {
      ...
      const bulkActionMod = await import("@/app/api/admin/bulk-action/route");
      const bulkActionHandler = wrapAppRouteHandler(bulkActionMod);
      ...
      expect(data.error).toBe("Initial closing threshold not met");
      expect(data.code).toBe("INITIAL_THRESHOLD_NOT_MET");
    ...
    it("allows capital call when threshold is met", async () => {
      ...
      (mockPrisma.capitalCall.create as jest.Mock).mockResolvedValue({
        id: "cc-1",
        fundId: "fund-1",
        callNumber: 1,
        amount: 100000,
    ...
    it("allows distributions regardless of threshold", async () => {
      ...
      (mockPrisma.distribution.create as jest.Mock).mockResolvedValue({
        id: "dist-1",
        fundId: "fund-1",
