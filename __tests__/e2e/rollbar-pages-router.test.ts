...
describe("Rollbar Pages Router Integration", () => {
  describe("clientConfig", () => {
    ...
    it("should have accessToken from environment or fallback", () => {
      // In CI, the env var may not be set, so token falls back to "disabled"
      const expectedToken = process.env.NEXT_PUBLIC_ROLLBAR_CLIENT_TOKEN || "disabled";
      expect(clientConfig.accessToken).toBe(expectedToken);
    });
  });
