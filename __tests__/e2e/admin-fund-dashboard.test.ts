...
import * as fundDashboardRoute from "@/app/api/admin/fund-dashboard/route";
import * as bulkActionRoute from "@/app/api/admin/bulk-action/route";

const fundDashboardHandler = wrapAppRouteHandler(fundDashboardRoute);
const bulkActionHandler = wrapAppRouteHandler(bulkActionRoute);
  ...
  describe("GP Access - Fund Dashboard", () => {
    ...
    beforeEach(() => {
      mockGetServerSession.mockResolvedValue({
        user: { id: "gp-user-1", email: "gp@example.com" },
      });
      ...
      const mockFunds = [
        {
          ...
          currentRaise: 500000,
          closingDate: null,
          _count: { capitalCalls: 1, distributions: 1 },
          aggregate: null,
        },
      ];

      (mockPrisma.fund.findMany as jest.Mock).mockResolvedValue(mockFunds);
      // Investment aggregation mocks
      (mockPrisma.investment.groupBy as jest.Mock)
        .mockResolvedValueOnce([
          { fundId: "fund-1", _sum: { commitmentAmount: 300000, fundedAmount: 150000 }, _count: { investorId: 2 } },
        ])
        .mockResolvedValueOnce([
          { fundId: "fund-1", investorId: "inv-1" },
          { fundId: "fund-1", investorId: "inv-2" },
        ]);
      (mockPrisma.distribution.groupBy as jest.Mock).mockResolvedValue([
        { fundId: "fund-1", _sum: { totalAmount: 25000 } },
      ]);
      (mockPrisma.manualInvestment.groupBy as jest.Mock).mockResolvedValue([]);
      ...
      (mockPrisma.fund.findMany as jest.Mock).mockResolvedValue([
        {
          ...
          currentRaise: 500000,
          closingDate: null,
          _count: { capitalCalls: 0, distributions: 0 },
          aggregate: null,
        },
      ]);
      (mockPrisma.investment.groupBy as jest.Mock)
    ...
    it("GP with no teams is denied access", async () => {
      ...
      const data = JSON.parse(res._getData());
      expect(data.error).toBe("You need to be a team member to access the fund dashboard");
    });
  });
...
describe("Bulk Action E2E", () => {
  ...
  describe("Validation", () => {
    ...
    it("rejects invalid allocation type", async () => {
      ...
      expect(res._getStatusCode()).toBe(400);
      const data = JSON.parse(res._getData());
      expect(data.error).toBe("Invalid allocation type");
    });
    ...
    it("rejects pro-rata with zero commitments", async () => {
      ...
      expect(res._getStatusCode()).toBe(400);
      const data = JSON.parse(res._getData());
      expect(data.error).toBe("Cannot use pro-rata allocation: no commitments found");
    });
  });
