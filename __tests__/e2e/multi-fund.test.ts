...
describe("Multi-Fund Features", () => {
  ...
  describe("Per-Fund Dashboard API", () => {
    it("returns fund details with aggregates and investors", async () => {
      ...
      const fundDetailHandler = wrapAppRouteHandler(await import("@/app/api/admin/fund/[id]/route"), { id: "fund-1" });
    ...
    it("returns 404 for non-existent fund", async () => {
      ...
      const fundDetailHandler = wrapAppRouteHandler(await import("@/app/api/admin/fund/[id]/route"), { id: "nonexistent-fund" });
