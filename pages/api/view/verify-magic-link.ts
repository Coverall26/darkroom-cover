  ...
  try {
    ...
    if (isValid) {
      ...
      res.setHeader("Set-Cookie", [
        `pm_vft_${linkId}=${hashedVerificationToken}; Path=/; HttpOnly; SameSite=Strict${secureFlag}; Expires=${oneHourFromNow.toUTCString()}`,
        `pm_email_${linkId}=${normalizedEmail}; Path=/; HttpOnly; SameSite=Strict${secureFlag}; Expires=${oneHourFromNow.toUTCString()}`,
        `pm_drs_flag_${linkId}=verified; Path=/view/${linkId}; SameSite=Strict${secureFlag}; Expires=${oneHourFromNow.toUTCString()}`,
      ]);
