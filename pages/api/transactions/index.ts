...
export default async function handler(
  ...
  if (!user) {
    return res.status(statusCode || 401).json({ error });
  }
...
async function handlePost(req: NextApiRequest, res: NextApiResponse, user: any) {
  try {
    ...
    if (!teamFunds.some((f) => f.id === fundId)) {
      return res.status(403).json({ error: "Fund not in your team" });
    }
    ...
    if (!investor.personaStatus || !["APPROVED", "VERIFIED"].includes(investor.personaStatus)) {
      ...
      return res.status(403).json({ 
        error: "KYC verification required before initiating transfers",
        code: "KYC_REQUIRED",
