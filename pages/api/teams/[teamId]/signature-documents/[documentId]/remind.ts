  ...
  try {
    ...
    if (document.status === "DRAFT") {
      return res.status(400).json({
        error: "Cannot send reminders for draft documents. Send the document first."
      });
    }

    if (document.status === "COMPLETED" || document.status === "VOIDED" || document.status === "EXPIRED") {
      return res.status(400).json({
        error: "Cannot send reminders for completed, voided, or expired documents."
      });
    }
    ...
    if (recipientsToRemind.length === 0) {
      return res.status(400).json({
        error: "No recipients need reminders"
      });
    }
