...
export default async function handler(
  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch existing exports" });
    }
  }
  ...
  try {
    ...
    if (team.plan.includes("free")) {
      return res
        .status(403)
        .json({ error: "This feature is not available for your plan" });
    }
    ...
    if (!exportJob) {
      return res.status(503).json({ error: "Export service temporarily unavailable" });
    }
