...
export default async function handle(
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      // Check if the content is a FundRoom platform URL
      // Match against known platform domains + tenant custom domains with /view/ path
      const platformHosts = [
        "fundroom.ai",
        process.env.NEXT_PUBLIC_APP_BASE_HOST,
        process.env.NEXTAUTH_URL ? new URL(process.env.NEXTAUTH_URL).hostname : null,
      ].filter(Boolean) as string[];
      const isPlatformUrl = platformHosts.some(
        (host) => agreement.content.includes(`${host}/view/`),
      );
      ...
      if (isPlatformUrl) {
        // Extract linkId from FundRoom URL
        const urlParts = agreement.content.split("/view/");
        if (urlParts.length < 2) {
          return res.status(400).json("Invalid FundRoom URL format");
        }
        ...
        if (!link || !link.document) {
          return res
            .status(404)
            .json("Document not found for the provided URL");
        }
