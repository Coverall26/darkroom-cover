...
export default async function handle(
  ...
  if (req.method === "POST") {
    ...
    if (name.length > 255) {
      return res.status(400).json({
        error: "Dataroom name is too long",
      });
    }

    if (!Array.isArray(folders) || folders.length === 0) {
      return res.status(400).json({
        error: "Folder structure is required",
      });
    }
    ...
    try {
      ...
      if (!hasAccess) {
        return res.status(403).json({
          error: "This feature requires a datarooms plan. Please upgrade to access AI-generated data rooms.",
        });
      }
      ...
      if (limits && !(isFreePlan && isFirstDataroom) && dataroomCount >= limits.datarooms) {
        return res
          .status(403)
          .json({ error: "You have reached the limit of datarooms" });
      }
