...
export default async function handler(
  ...
  try {
    ...
    if (!link.teamId || !link.team) {
      return res.status(400).json({ error: "Invalid link configuration" });
    }
    ...
    for (const adminEmail of adminEmails) {
      try {
        ...
        const approvalUrl = magicLinkResult?.magicLink || `${baseUrl}/admin/login?next=${encodeURIComponent(redirectPath)}`;
