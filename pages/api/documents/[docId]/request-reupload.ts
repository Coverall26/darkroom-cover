 ...
 *
 * GP requests the LP to re-upload a document.
 * Sets status to REVISION_REQUESTED and sends email to LP.
 */
  ...
  try {
    ...
    if (lpDocument.status !== "UPLOADED_PENDING_REVIEW") {
      return res.status(400).json({
        error: "Only pending documents can have re-upload requested",
      });
    }
    ...
    await logAuditEvent({
      ...
      resourceId: docId,
      metadata: {
        action: "GP requested document re-upload",
    ...
    // Fire-and-forget: notify LP about re-upload request
    sendReviewNotification(
      ...
      console.error("[DOC_REUPLOAD] Failed to send review notification:", err);
    });
    ...
    console.error("[DOC_REQUEST_REUPLOAD] Error:", error);
...
async function sendReviewNotification(
  ...
  try {
    ...
    console.error("[DOC_REQUEST_REUPLOAD] Failed to send notification:", error);
  }
}
