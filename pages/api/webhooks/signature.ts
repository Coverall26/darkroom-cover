...
async function handleDocumentSigned(req: NextApiRequest, documentId: string, recipientId: string) {
  ...
  const allSigned = document.recipients.every((r: { status: string }) => r.status === "SIGNED");
...
async function sendCompletionNotification(document: any, investor: any) {
  ...
  if (document.team) {
    ...
    teamAdmins.forEach((admin: { user: { email: string | null } }) => {
      if (admin.user.email) {
  ...
  for (const email of [...new Set(recipients)]) {
    try {
      ...
      console.error("[SIGNATURE_WEBHOOK] Failed to send notification:", error);
    }
  }
}
