...
export async function checkoutSessionCompleted(
  ...
  // Clear cached tier so next request picks up the new plan immediately
  clearTierCache(teamId);

  // Fire-and-forget server-side funnel event (no PII)
  publishServerEvent("funnel_upgrade_completed", {
    teamId,
    plan: plan.slug,
    priceId,
  });
