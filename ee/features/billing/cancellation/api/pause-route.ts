...
export async function handleRoute(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "POST") {
    ...
    try {
      ...
      // Clear cached tier so paused capabilities take effect immediately
      ...
      waitUntil(
        Promise.all([
          ...
          sendPauseResumeNotificationTask.trigger(
            { teamId },
            {
              delay: { until: reminderAt }, // 3 days before pause ends
              tags: [`team_${teamId}`],
          ...
          automaticUnpauseTask.trigger(
            { teamId },
            {
              delay: { until: pauseEndsAt }, // Exactly when pause period ends
              tags: [`team_${teamId}`],
