...
export function DataroomLinkSheet({
  ...
  const handlePreviewLink = async (link: LinkWithViews) => {
    ...
    try {
      ...
      const marketingUrl = process.env.NEXT_PUBLIC_MARKETING_URL || (typeof window !== 'undefined' ? window.location.origin : 'https://app.fundroom.ai');
  ...
  const handlePostSaveOperations = async (
    ...
    if (isUpdating) {
      setIsOpen(false);
      // Update the link in the list of links and revalidate from server
      mutate(
      ...
      // Handle group changes - revalidate group-specific links cache
      if (!!groupId) {
        mutate(
          `/api/teams/${teamId}/${endpointTargetType}/${encodeURIComponent(
            targetId,
