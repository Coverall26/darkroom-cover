...
import type { ItemType, PermissionGroupAccessControls } from "@prisma/client";
import {
  ColumnDef,
  flexRender,
  getCoreRowModel,
...
const PermissionGroupItemName = ({ item }: { item: FileOrFolder }) => {
  ...
  return (
    <div className="flex items-center text-foreground">
      ...
      ) : item.itemType === ("DATAROOM_FOLDER" as ItemType) ? (
  ...
  items
    ...
    .forEach((folder) => {
      ...
      const folderDocuments = (folder.documents || []).map((doc: any) => ({
        ...
        itemType: ("DATAROOM_DOCUMENT" as ItemType),
      }));
      ...
      result.push({
        ...
        itemType: ("DATAROOM_FOLDER" as ItemType),
      });
    });
        ...
        const updateItemInTree = (items: FileOrFolder[]): FileOrFolder[] => {
          return items.map((currentItem) => {
            if (currentItem.id === id) {
              ...
              if (updatedItem.itemType === ("DATAROOM_FOLDER" as ItemType)) {
