    ...
    const viewersWithLinks = participants
      .map((participant) => {
        ...
        if (link.domainId && link.domainSlug && link.slug) {
          ...
          linkUrl = `${process.env.NEXT_PUBLIC_MARKETING_URL || 'https://app.fundroom.ai'}/view/${link.id}`;
        }
    ...
    for (const viewer of viewersWithLinks) {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL || 'https://app.fundroom.ai'}/api/jobs/send-conversation-new-message-notification`,
          {
            method: "POST",
            body: JSON.stringify({
              conversationId: payload.conversationId,
...
export const sendConversationTeamMemberNotificationTask = task({
  ...
  run: async (payload: NotificationPayload) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL || 'https://app.fundroom.ai'}/api/jobs/send-conversation-team-member-notification`,
        {
          method: "POST",
          body: JSON.stringify({
            conversationId: payload.conversationId,
