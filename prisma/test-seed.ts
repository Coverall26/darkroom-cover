...
async function main() {
  console.log("\n🧪 FundRoom AI Platform - Test Data Seeding");
  console.log("========================================\n");
  ...
  const testAdminEmail = "test-admin@fundroom.test";
  const testLpEmail = "test-lp@fundroom.test";
  const testViewerEmail = "test-viewer@fundroom.test";
  ...
  let pendingInvestor = await prisma.investor.findFirst({
    where: { user: { email: "test-pending-lp@fundroom.test" } },
  });
  if (!pendingInvestor) {
    const pendingUser = await prisma.user.create({
      data: {
        email: "test-pending-lp@fundroom.test",
  ...
  console.log(`   Pending:   test-pending-lp@fundroom.test`);
  console.log(`\n   Team:      ${team.name}`);
...
async function cleanTestData() {
  const testEmails = [
    "test-admin@fundroom.test",
    "test-lp@fundroom.test",
    "test-viewer@fundroom.test",
    "test-pending-lp@fundroom.test",
  ];
