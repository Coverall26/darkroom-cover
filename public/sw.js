const CACHE_VERSION = 'v5-4c73f6be7a4cf1b2';
const CACHE_NAME = `fundroom-${CACHE_VERSION}`;
const STATIC_CACHE_NAME = `fundroom-static-${CACHE_VERSION}`;
const DYNAMIC_CACHE_NAME = `fundroom-dynamic-${CACHE_VERSION}`;
const DOCUMENT_CACHE_PREFIX = 'fundroom-documents-';
...
const DB_NAME = 'fundroom-offline';
const DB_VERSION = 2;
...
async function cacheDocument(docInfo) {
  ...
  try {
    ...
    return { success: false, error: 'Failed to cache document' };
  }
}
  ...
  event.waitUntil(
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames
          .filter((name) => {
            return name.startsWith('fundroom-') && 
                   !name.includes(CACHE_VERSION) &&
                   !name.startsWith('fundroom-documents-');
          })
          .map((name) => {
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.startsWith('/api/')) {
    // NEVER cache sensitive/financial API responses — pass through to network
    if (request.headers.get('Authorization') ||
        ...
        url.pathname.includes('/investors') ||
        url.pathname.includes('/funds') ||
          ...
          for (let i = 0; i < retries; i++) {
            try {
              ...
              if (networkResponse.status === 429) {
                console.log('[SW] Rate limited (429), attempt', i + 1, 'of', retries, url.pathname);
                if (cachedResponse) {
                  console.log('[SW] Serving cached version due to rate limit:', url.pathname);
                  return cachedResponse;
                }
                if (i < retries - 1) {
        ...
        async function fetchWithRetry(retries = 3, delay = 1000) {
          for (let i = 0; i < retries; i++) {
            try {
              const networkResponse = await fetch(request);
              if (networkResponse.status === 429) {
                console.log('[SW] Rate limited (429) for asset, attempt', i + 1, url.pathname);
                if (cachedResponse) {
                  return cachedResponse;
                }
                if (i < retries - 1) {
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    const options = {
      ...
      icon: '/icons/fundroom/icon-192x192.png',
      badge: '/icons/fundroom/icon-72x72.png',
      vibrate: [100, 50, 100],
      data: {
        url: data.url || '/',
      },
    ...
    event.waitUntil(
      self.registration.showNotification(data.title || 'FundRoom', options)
    );
  } catch (error) {
...
// Background sync stub — Phase 2: wire to pending offline actions queue
self.addEventListener('sync', (event) => {
  if (event.tag === 'sync-pending-actions') {
    // No-op for V1 — offline action queue not yet implemented
