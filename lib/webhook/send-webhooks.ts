...
const publishWebhookEventToQStash = async ({
  ...
  // Skip if QStash is not configured
  if (!qstash) {
    console.warn("QStash client not configured - skipping webhook publish");
    return { messageId: null };
  }

  // Callback URL uses NEXT_PUBLIC_BASE_URL (app.fundroom.ai in production)
  ...
  const response = await qstash.publishJSON({
    ...
    body: payload,
    headers: {
      "X-FundRoom-Signature": signature,
