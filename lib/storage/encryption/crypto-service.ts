...
export class CryptoService {
  ...
  constructor(encryptionKey?: string) {
    ...
    const isValidHexKey = hexBuffer.length === 32 && /^[0-9a-fA-F]{64}$/.test(keySource);
    ...
    if (isValidHexKey) {
      ...
      throw new Error(
        "SECURITY ERROR: Password-derived encryption keys are not allowed in production. " +
        "Please set STORAGE_ENCRYPTION_KEY to a 64-character hex string (32 bytes). " +
        "Generate one with: node -e \"console.log(require('crypto').randomBytes(32).toString('hex'))\""
      );
    } else {
      const stableDerivationSalt = crypto.createHash("sha256").update("fundroom-master-key-derivation").digest();
      this.masterKey = this.deriveKey(keySource, stableDerivationSalt);
      this.useDerivedKeys = true;
      console.warn(
        "⚠️  CryptoService: Using password-based key derivation (DEVELOPMENT ONLY). " +
        "For production, set STORAGE_ENCRYPTION_KEY to a 64-character hex key."
      );
    }
  }
  ...
  private deriveObjectKey(salt: Buffer): Buffer {
    ...
    const derived = crypto.hkdfSync("sha256", this.masterKey, salt, "fundroom-storage", 32);
