...
async function createDataroomSession(
  ...
  if (!redis) {
    ...
    console.warn("Redis not configured - dataroom sessions will not persist across page loads");
    return null;
  }
...
async function verifyDataroomSession(
  ...
  let session;
  try {
    session = await redis.get(`dataroom_session:${sessionToken}`);
  } catch (error) {
  ...
  try {
    ...
    console.error("[DATAROOM_AUTH] Session validation error:", error);
