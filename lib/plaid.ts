 ...
 * Plaid is Phase 2 — MVP uses manual wire transfer only.
...
function getEncryptionKey(): string {
  const baseKey = process.env.PLAID_TOKEN_ENCRYPTION_KEY || process.env.NEXTAUTH_SECRET;
  if (!baseKey && process.env.NODE_ENV === 'production') {
    throw new Error('PLAID_TOKEN_ENCRYPTION_KEY is required in production');
  }
  if (!process.env.PLAID_TOKEN_ENCRYPTION_KEY && process.env.NODE_ENV === 'production') {
    console.warn('[SECURITY] Using NEXTAUTH_SECRET as Plaid encryption fallback. Set PLAID_TOKEN_ENCRYPTION_KEY for production.');
  }
  const PURPOSE_SALT = 'fundroom-plaid-token-encryption-v1';
  return baseKey ? `${baseKey}:${PURPOSE_SALT}` : 'default-key-for-dev-only';
}
...
/**
 * @deprecated Use isPlaidEnabled() instead which also checks the PLAID_ENABLED feature flag.
 */
export function isPlaidConfigured(): boolean {
  return !!(process.env.PLAID_CLIENT_ID && process.env.PLAID_SECRET);
}

export async function createLinkToken(userId: string, clientName: string = 'FundRoom'): Promise<string> {
