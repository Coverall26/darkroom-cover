...
export const processDocument = async ({
  ...
  const document = await prisma.document.create({
    data: {
      ...
      encryptionKeyHash: encryptionKeyHash ?? null,
  ...
  // Only run if Trigger.dev is configured
  ...
  if (triggerConfigured) {
    try {
      ...
      console.warn("Document conversion task skipped (Trigger.dev error):", error);
    }
  }
  ...
  if (type === "pdf") {
    if (process.env.TRIGGER_SECRET_KEY) {
      try {
        ...
        console.warn("Trigger.dev PDF conversion failed, falling back to local processing");
