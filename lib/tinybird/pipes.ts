...
if (!process.env.TINYBIRD_TOKEN) {
  console.warn("TINYBIRD_TOKEN not configured - analytics features will be limited");
}

const tb = new Tinybird({ 
  token: process.env.TINYBIRD_TOKEN || "",
  baseUrl: process.env.TINYBIRD_HOST || "https://api.us-west-2.aws.tinybird.co",
});
...
export const getSignatureEventsByDocument = tb.buildPipe({
  pipe: "get_signature_events_by_document__v1",
  parameters: z.object({
    documentId: z.string(),
...
export const getServerEvents = tb.buildPipe({
  pipe: "get_server_events__v1",
  parameters: z.object({
    eventName: z.string(),
...
export const getServerEventCounts = tb.buildPipe({
  pipe: "get_server_event_counts__v1",
  parameters: z.object({
    since: z.number(),
    teamId: z.string().optional(),
    orgId: z.string().optional(),
