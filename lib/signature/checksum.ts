...
export function generateVerificationToken(
  ...
  const baseSecret = process.env.SIGNATURE_VERIFICATION_SECRET || process.env.NEXTAUTH_SECRET;
  if (!baseSecret) {
    throw new Error("SIGNATURE_VERIFICATION_SECRET or NEXTAUTH_SECRET is required");
  }
  if (!process.env.SIGNATURE_VERIFICATION_SECRET && process.env.NODE_ENV === "production") {
    throw new Error(
      "SECURITY ERROR: SIGNATURE_VERIFICATION_SECRET is required in production. " +
      "Do not fall back to NEXTAUTH_SECRET for signature verification."
    );
  }
  const PURPOSE_SALT = "fundroom-signature-verification-v1";
  const derivedSecret = crypto.createHash("sha256").update(`${baseSecret}:${PURPOSE_SALT}`).digest("hex");
