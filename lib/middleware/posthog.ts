...
export default async function PostHogMiddleware(req: NextRequest) {
  ...
  // If PostHog host is not configured, disable proxying entirely — no data leaves the platform
  if (!posthogHost) {
    return new NextResponse("Not Found", { status: 404 });
  }

  let url = req.nextUrl.clone();
  const hostname = url.pathname.startsWith("/ingest/static/") && posthogAssetsHost
    ? posthogAssetsHost
    : posthogHost;
