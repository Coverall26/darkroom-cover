...
/**
 * Validates and sanitizes a Notion media URL to prevent path traversal and injection attacks.
 * Only allows known Notion domains and attachment: protocol.
 * Returns null if URL is invalid or potentially malicious.
 */
function validateNotionMediaUrl(source: string | undefined | null): string | null {
  if (!source || typeof source !== "string") {
    return null;
  }

  // Trim and limit length to prevent DoS
  const trimmedSource = source.trim().slice(0, 2048);
  
  // Block path traversal sequences
  if (trimmedSource.includes("..") || trimmedSource.includes("\0")) {
    return null;
  }

  // Handle attachment: protocol (internal Notion references)
  if (trimmedSource.startsWith("attachment:")) {
    // Validate attachment format: attachment:<uuid> or similar safe pattern
    const attachmentPattern = /^attachment:[a-zA-Z0-9\-_]+$/;
    if (attachmentPattern.test(trimmedSource)) {
      return trimmedSource;
    }
    return null;
  }

  // Validate as URL for external sources
  try {
    const url = new URL(trimmedSource);
    
    // Only allow HTTPS and specific Notion domains
...
export const addSignedUrls: NotionAPI["addSignedUrls"] = async ({
  ...
  const allFileInstances = contentBlockIds.flatMap((blockId) => {
    ...
    if (
      ...
      // Extract source URL from block data
      const rawSource =
        block.type === "page"
          ? block.format?.page_cover
          : block.properties?.source?.[0]?.[0];

      // Validate and sanitize the source URL to prevent path traversal/injection
      const source = validateNotionMediaUrl(rawSource);

      if (source) {
        // Only process validated URLs from known Notion domains
        if (
