      ...
      async (path) => {
        ...
        if (path.startsWith("https://")) {
          try {
            ...
            // Also check for standard Vercel Blob URLs
            if (hostname.endsWith(".blob.vercel-storage.com") || hostname.endsWith(".public.blob.vercel-storage.com")) {
              isVercelBlob = true;
            }
...
export const documentUploadSchema = z
  .object({
    ...
    isClientEncrypted: z.boolean().optional(),
    encryptionKeyHash: z.string().optional(),
    encryptionIv: z.string().optional(),
    originalFileName: z.string().optional(),
    originalFileSize: z.number().int().positive().optional(),
  })
  .refine(
    (data) => {
        ...
        if (data.url.startsWith("https://")) {
          try {
            ...
            const isVercelBlob = 
              ...
              (process.env.VERCEL_BLOB_HOST && hostname.startsWith(process.env.VERCEL_BLOB_HOST));
